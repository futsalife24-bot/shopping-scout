...
import { cropAndRotate, inspectImageQuality, loadImage, type CropRect, type ImageQuality } from '@/features/ocr/imageTools';
import { recognizeLabel } from '@/features/ocr/tesseractClient';
...
export function App() {
  ...
  const selectImage = (file?: File) => {
    ...
    reader.onload = () => void setImageForCrop(String(reader.result));
  ...
  return (
    <main className="page">
      ...
      {step === 'home' && <section className="card home-card">
        ...
        <div className="home-actions"><button className="button primary" onClick={() => void startCamera()}>値札を撮る</button><label className="button secondary">画像を選ぶ<input type="file" accept="image/*" capture="environment" onChange={(event) => selectImage(event.target.files?.[0])} /></label></div>
